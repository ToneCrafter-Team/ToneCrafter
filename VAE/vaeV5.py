...
nbrVariables=2 #si !=2 pas de plot en 2D  || Il s'agit du nombre de variables de l'espace latent 
...
#Test translation dans l'espace latent
    ...
    image_base = generator_model.predict(latent_point)[0]
    x2=x[i]+vecteurTranslationX
    y2=y[i]+vecteurTranslationY
    latent_point = np.array([[x2, y2]])
    image_trans = generator_model.predict(latent_point)[0]
